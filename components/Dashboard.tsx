...
import { Bell, LogOut, CheckCircle, AlertCircle, Heart, BellOff, Loader2, Shield, TrendingUp, X, Check } from 'lucide-react';
...
export default function Dashboard({ user, onLogout, refreshProfile, onNavigate }: DashboardProps) {
  ...
  const handleOneTimeDonation = async () => {
    setIsUpdating(true);
    const priceId = PRICE_IDS.DONATION_5;

    if (isMock) {
        setTimeout(() => {
            setLocalUser(prev => ({...prev, lastDonationDate: new Date().toISOString()}));
            setIsUpdating(false);
            alert(`[SIMULACIÓN] ¡Donación puntual de 5€ recibida! Ya puedes votar.`);
        }, 1500);
    } else {
        const result = await initiateCheckout(priceId, user.id, 'payment');
        if (result.success) {
            await supabase.from('profiles').update({ last_donation_date: new Date().toISOString() }).eq('id', user.id);
            setIsUpdating(false);
            alert('¡Donación puntual de 5€ recibida! Ya puedes votar.');
            refreshProfile();
        } else {
            setIsUpdating(false);
  ...
  const handleSubscriptionSelect = async (tier: SubscriptionTier) => {
    ...
    // Determine Price ID
    const key = `${tier.toUpperCase()}_${pricingMode.toUpperCase()}`; 
    // @ts-ignore - Dynamic access to PRICE_IDS
    const priceId = PRICE_IDS[key] || PRICE_IDS.BRONZE_SIMPLE;
    
    const successMessage = `¡Suscripción ${tier.toUpperCase()} (${pricingMode}) activada!`;
    ...
    if (isMock) {
         setTimeout(() => {
            ...
            setIsUpdating(false);
            alert(`[SIMULACIÓN] ${successMessage}`);
        }, 1500);
    } else {
